...
export default async (req : Request, res : Response) => {
	if (req.body.description === undefined || req.query.id === undefined){
		...
		if (await RecruitmentApplyService(userId, recruitmentId, req.body.description)){
			res.status(statusCode.OK).send(util.success(statusCode.OK, "apply success"))
		}else{
			res.status(statusCode.BAD_REQUEST).send(util.fail(statusCode.BAD_REQUEST, "duplicated user input"))
		}
		return
	}
}